                    ...
                    <div class="col-lg-6">
                     ...
                     <div class="col-lg-6">
                        <form action="action/post_notify.php" method="POST">
                            ...
                            <div class="form-group">
                                <select id="school-select" class="school form-control select" name="school[]" multiple="multiple">
                                    <option value="all">Select ALL</option>
                                    <?php
                                    $result = mysqli_query($conn, "SELECT * FROM school_list");
                                    while ($user_row = mysqli_fetch_array($result)) {
                                        echo "<option value='" . $user_row['school_id'] . "'>" . $user_row['school_name'] . "</option>";
                                    }
                                    ?>
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="tri_message">Message box</label>
                                <textarea class="form-control" id="sch_message" name="sch_message" rows="5" style="resize: none;"></textarea>
                            </div>
                            <button type="submit" name="sch_submit" class="send btn btn-success">Send</button>
                        </form>
                    </div>
                </div>
            </div>
    ...
    $(document).ready(function() {
        ...
        $('#trainer-select').on('change', function() {
         ...
         $('#school-select').on('change', function() {
            if ($(this).val().includes('all')) {
                let allSelected = $('#school-select > option').map(function() {
                    return $(this).val();
                }).get();
                $('#school-select').val(allSelected).trigger('change');
            }
        });
    });
</script>
