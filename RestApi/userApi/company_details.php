...
if (!empty($data->api_key)) {
    ...
    if ($itemCount > 0) {
        ...
        $stmt = $user->company_details();
        $itemCount = $stmt->rowCount();

        if ($itemCount > 0) {
            $terms_cons = array();
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                $terms_cons[] = $row['company_details'];
            }
            echo json_encode(array("status" => true, "message" => "company details", "Data" => $terms_cons));
        } else {
            http_response_code(400);
            echo json_encode(array("status" => false, "message" => " company details Found"));
        }
    } else {
