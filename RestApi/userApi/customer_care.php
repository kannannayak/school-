...
if (!empty($data->api_key)) {
    ...
    if ($itemCount > 0) {
        ...
        $stmt = $user->customercare();
        $itemCount = $stmt->rowCount();

        if ($itemCount > 0) {
            $terms_cons = array();
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                $terms_cons[] = $row['customer_care'];
            }
            echo json_encode(array("status" => true, "message" => "Customer care message", "Data" => $terms_cons));
        } else {
            http_response_code(400);
            echo json_encode(array("status" => false, "message" => "No Customer care message"));
        }
    } else {
